...
I finally found a workaround: inject *-device* and *-drive* settings via the *qemuargs* property. What surprises me is that parameters in the *qemuargs* property are not *appended* to the ones generated by packer (via iso_url etc); they override them. What this means is that we would have to re-hook up devices (cdrom and disk), forgeting what we have already set cdrom_interface, disk_interface, iso_url etc. 

The way we inject QEMU parameters via the *qemuargs* property works very well except when the iso_url points to an http/https url. In such case, it doesn't work at all. As such, we would have to download an .iso and save it in local file system and modify the *iso_url* to point to it.

I'm not sure whether it would be a bug or a feature that *packer* could not generate two *sda* drives without applying *qemuargs*. 

What is very interesting is that on Linux (including WSL2) the Photon OS 5.0 installer doesn't seem to require that CDROM be *sata* interface; an *ide* is fine! As such, I don't need to hack the *qemuargs* property. The following two scenarios are working fine for me when packer is run on Linux:
* CDROM uses *ide* and disk uses *virtio-scsi* (the hard disk will be /dev/sata at installation time)
* CDROM uses *ide* and disk uses *virtio* (the hard disk will be /dev/vda at installation time)

Is it to do with QEMU version difference (the version of QEMU for Windows is 9.2.0 whereas linux equivalent is 8.2.2) that drives generated by packer are inconsist between Windows and Linux environment? I don't know.
